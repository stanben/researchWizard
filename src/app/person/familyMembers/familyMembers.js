(function () {
	...
	rwApp.directive('rwFamilyMembers',['fsApi', 'fsUtils', 'fsLocation', 'rwDrw', 
		function (fsApi, fsUtils, fsLocation, rwDrw) {
		return {
			templateUrl: 'person/familyMembers/familyMembers.tpl.html',
			scope: {
			...
			link: function (scope) {
				...
				if (scope.family.children == null ||
					...
					fsApi.getPersonWithRelationships(primaryId, { persons: true }).then(function (response) {
						...
						if (scope.focusId === scope.family.husband.id || scope.focusId === scope.family.wife.id) {
							rwDrw.spouseFamily(scope.focusId,scope.family,scope.couple,scope.children);
						} else {
							rwDrw.parentFamily(scope.family, scope.couple, scope.children);
						}
					});
				}
				else {
				...
				/*
							scope.addChildHref = fsLocation.getFindAddUrl(3.removeEmptyProperties({
