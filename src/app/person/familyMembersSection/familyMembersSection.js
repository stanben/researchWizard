(function () {
	...
	rwApp.directive('rwFamilyMembersSection', function (_, $rootScope, fsApi, fsUtils /*, fsLocation*/) {
			...
			return {
				templateUrl: 'person/familyMembersSection/familyMembersSection.tpl.html',
				scope: {
					state: '=',
					person: '='
				},
				link: function (scope) {
					...
					fsApi.getPersonWithRelationships(scope.person.id, { persons: true }).then(function (response) {
						...
						console.log('getPersonWithRelationships.response: ' + response);
					...
					fsApi.getPreferredSpouse(scope.person.id).then(function (response) {
						...
						console.log('getPreferredSpouse.response: ' + response);
						console.log('scope: ');
						console.log(scope);
					...
					fsApi.getPreferredParents(scope.person.id).then(function (response) {
						...
						console.log('getPreferredParents.response: ' + response);
					});

					scope.showPreferred = function (family, families) {
						console.log('showPreferred: ' + family + families);
					...
					scope.$on('save', function (event, relationshipId, isSet) {
						console.log('fsFamilyMembersSection scope.$on("save"');
...
/*
					...
					console.log('scope.addSpouseHref: ' + scope.addSpouseHref);
					...
					console.log('scope.addChildHref: ' + scope.addChildHref);
...
*/
					console.log('scope.addParentHref: ' + scope.addParentHref);
				}
			};
		});
})();